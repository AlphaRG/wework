...
- (void)drawMoreGridView
{
    ...
    highlightedImage = [UIImage imageNamed:@"report_pressed_bg"];
    UIImage *deleteIconImage = [UIImage imageNamed:@"report_add"];
    
    if (_showGridIDArray.count ==0) {
        
        UIView *noAppView = [[UIView alloc]initWithFrame:CGRectMake(ScreenWidth/2-15, ScreenHeight/2, 30, 30)];
        UILabel *labletitle = [[UILabel alloc]initWithFrame:CGRectMake(ScreenWidth/2-50, ScreenHeight/2 -150, 100, 21)];
        labletitle.text = @"暂无其他报表";
        labletitle.textColor = [UIColor grayColor];
        labletitle.font = [UIFont systemFontOfSize:13];

        noAppView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@""]];
        
        [_showMoreGridView addSubview:noAppView];
        [_showMoreGridView addSubview:labletitle];
    }
    else{
